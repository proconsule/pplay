...
InfoBox::InfoBox(Main *m, const c2d::Vector2f &position)
...
*/
    ...
    messageText->setPosition(16,
                             titleText->getPosition().y + main->getFontSize(Main::FontSize::Medium) + 4);
    messageText->setSizeMax(getSize().x - 16, 0);
    add(messageText);
...
void InfoBox::show(const std::string &title, const std::string &message, bool inf, bool drawNow) {
...
void InfoBox::onDraw(c2d::Transform &transform, bool draw) {
...
void InfoBox::setAlpha(uint8_t alpha, bool recursive) {
